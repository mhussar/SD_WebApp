...
def ftp_image(p_image_file):
    ...
    image_directory = 'D:\\Projects\\Personal\\AI_ImageCreation\\StableDiffusion\\stable-diffusion-main\\outputs\\img-samples\\'

    image_path = image_directory + p_image_file

    #set up session with host, user and pass
    session = ftplib.FTP('ftpupload.net','epiz_32572577','a6zy63LRSF')

    #change to the image folder
    session.cwd('/htdocs/SD')
    ...
    if 'image.png' in session.nlst():
        print('yes for tut')
        session.delete('/htdocs/SD/image.png')
    else:
        print ('no')
    ...
    session.storbinary('STOR /htdocs/SD/image.png', file)
...
pnconfig.subscribe_key = 'sub-c-1d29f340-d1cb-11e6-b2ab-0619f8945a4f'
pnconfig.publish_key = 'pub-c-aea2344e-e80b-48f2-a63b-53fb81bc81f2'
pnconfig.uuid = 'pi'
...
def read_web_prompt_text_file():
    with open('webPromptTextFile.txt') as f:
        contents = f.read()
        f.close()
        return contents
...
def clear_web_prompt_text_file():
    with open('webPromptTextFile.txt', "w") as f:
        pass
        f.close()


def write_to_web_prompt_text_file(prompt):
    prompt_file = open('webPromptTextFile.txt', 'w')
...
def main():
    ...
    pubnub.subscribe().channels('Channel-SD').execute()
    #---------------END PUBNUB MODS--------------------------------

    try:

        #---------------BEGIN MODS FOR WEB APP ------------------------------
        web_prompt_text_file = 'D:\Projects\Personal\AI_ImageCreation\StableDiffusion\stable-diffusion-main\webPromptTextFile.txt'
