#!/usr/bin/env python3
# Copyright (c) 2022 Hitlab Studios (https://github.com/lstein)
...
def ftp_image(p_image_file):

    image_directory = 'D:\\Projects\\Personal\\AI_ImageCreation\\StableDiffusion\\stable-diffusion-main\\outputs\\img-samples\\' #change this to the path on your computer

    image_path = image_directory + p_image_file

    #set up session with host, user and pass
    session = ftplib.FTP('host','user','pass')

    #change to the image folder
    session.cwd('/your_web_server_root_for_html/SD')

    #get a list of all files in the ftp image folder
    filesinRemoteFolder = session.nlst()

    #check if file exists in the ftp image folder and if it does delete it
    if 'image.png' in session.nlst():
        print('yes for tut')
        session.delete('/your_web_server_root_for_html/SD/image.png')
    else:
        print ('no')
    ...
    session.storbinary('STOR /your_web_server_root_for_html/SD/image.png', file)
...
pnconfig.subscribe_key = 'sub-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
pnconfig.publish_key = 'pub-c-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'
pnconfig.uuid = 'pi'
pubnub = PubNub(pnconfig)


#for handling the dictionary that is returned by pubnub
...
class MySubscribeCallback(SubscribeCallback):
    ...
    def status(self, pubnub, status):
        if status.category == PNStatusCategory.PNUnexpectedDisconnectCategory:
            ...
            #pubnub.publish().channel('Channel-SD').message('').pn_async(my_publish_callback)
    ...
    def message(self, pubnub, message):
        ...
        # prompt from pubnub is parsed and written to local file
        write_to_web_prompt_text_file(parse_message(message.message))
...
def read_web_prompt_text_file():
    with open('webPromptTextFile.txt') as f:  #webPromptTextFile.txt  should be placed in the same folder as this python file, dream.py
        contents = f.read()
        f.close()
        return contents


def clear_web_prompt_text_file():
    with open('webPromptTextFile.txt', "w") as f:  #webPromptTextFile.txt  should be placed in the same folder as this python file, dream.py
        pass
        f.close()


def write_to_web_prompt_text_file(prompt):
    prompt_file = open('webPromptTextFile.txt', 'w')  #webPromptTextFile.txt  should be placed in the same folder as this python file, dream.py
...
def main():
    ...
    pubnub.subscribe().channels('Channel-SD').execute()  #make sure this channel , 'Channel-SD' is the same in SD.html
    #---------------END PUBNUB MODS--------------------------------

    try:

        #---------------BEGIN MODS FOR WEB APP ------------------------------
        web_prompt_text_file = 'D:\Projects\Personal\AI_ImageCreation\StableDiffusion\stable-diffusion-main\webPromptTextFile.txt'    #change path to match your computer set up
        #---------------END MODS FOR WEB APP ------------------------------
    ...
    with open(log_path, 'a') as log:
        ...
        main_loop(t2i, opt.outdir, cmd_parser, log, web_prompt_text_file)
        log.close()
...
def main_loop(t2i, outdir, parser, log, web_prompt_text_file):
    ...
    while not done:
        try:
            command = read_web_prompt_text_file()
            while len(command)<= 1:
                command = read_web_prompt_text_file()


            clear_web_prompt_text_file()
        ...
        if web_prompt_text_file and len(command) == 0:
            done = True
            break
        ...
        #send image file name to ftp function after parsing it
        imageFileName = results[0]
        imageFileName = imageFileName[0]
        imageFileName = imageFileName[20:]
        ftp_image(imageFileName)
